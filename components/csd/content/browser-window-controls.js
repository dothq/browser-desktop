...
class BrowserWindowControls extends MozHTMLElement {
		...
		switch (event.target) {
			...
			case this.elements.restore:
				return window.fullScreen
					? console.log("todo: exit fullscreen")
					: window.restore();
	...
	/**
	 * Dispatches a CSD position update
	 */
	dispatchPositionUpdate() {
		const evt = new CustomEvent("BrowserWindowControls::UpdatePosition", {
			detail: this.csdReversedMediaQuery.matches,
			bubbles: true,
			composed: true
		});
	...
	connectedCallback() {
		...
		this.csdReversedMediaQuery = window.matchMedia(
		...
		// Observe the document for changes to the chromemargin attribute
		// and recompute the visibility of CSD when this happens.
		const observer = new MutationObserver(this.maybeShow.bind(this));
	...
	/**
	 * Determines whether we need to hide or show the window CSD
	 */
	maybeShow() {
		this.hidden = !document.documentElement.hasAttribute("chromemargin");
	}
	...
	disconnectedCallback() {
		...
		this.csdReversedMediaQuery.removeEventListener("change", this);
