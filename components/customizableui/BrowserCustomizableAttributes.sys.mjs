...
const PrimitiveTypes = {
	...
	namedColor: (attribute) => (value) => {
		...
		return PrimitiveTypes.color(attribute)(value);
	},
...
export const BrowserCustomizableAttributes = {
	/**
	 * The attributes map for all customizable attributes
	 */
	get attributesMap() {
		return {
			width: PrimitiveTypes.flexibleDimension("width"),
			height: PrimitiveTypes.flexibleDimension("height"),

			background: PrimitiveTypes.namedColor("background"),
			text: PrimitiveTypes.namedColor("text"),

			orientation: PrimitiveTypes.orientation("orientation"),

			mode: PrimitiveTypes.mode("mode"),

			is: PrimitiveTypes.string("is")
		};
	},
	...
	processAttribute(attributeName, value) {
		const caster = this.attributesMap[attributeName];

		if (!caster) {
			throw new Error(
				`Attribute '${attributeName}' does not relate to a valid attribute.`
			);
		}
