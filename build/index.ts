...
import { error, info, success, warning, yesno } from "./utils/log";
...
export class Melon {
	...
	public async init() {
		...
		this.nodePackageManager = (await lookpath("yarn")) ? "yarn" : "npm";
	}
	...
	public getVersionString() {
		...
		for (const [key, value] of Object.entries(this.versions)) {
			const name = (
				Object.entries(versionNameBinding).find((i: any) =>
					i[1].includes(key)
				) as any
			)[0];
			const text = `\n\t${name}: ${value}`;
		...
		for (const cmd of commands) {
			...
			this.yargs.command({
				...
				aliases: Command.aliases
					? Command.aliases.filter((i: any) => i !== Command.name)
					: [],
				description: Command.description,
				handler: async (args: any) => {
					process.env.MOZ_SOURCE_CHANGESET = (
						await $$({ shutUp: true })`git rev-parse HEAD`
					).data;
					process.env.__MELON__ = "1";
					process.env.MOZBUILD_STATE_PATH = resolve(
						process.cwd(),
						".melon",
						"mozbuild"
					);
	...
	public maybeShowDeprecation() {
		if (existsSync(resolve(process.cwd(), ".dotbuild"))) {
			this.warning(
			...
			this.warning(`since the project was setup on the older version.`);
			this.warning(``);
			this.warning(
			...
			this.warning(`so just delete it from the terminal instead.`);
		}
	}

	public maybeMakeMozconfig() {
		if (!existsSync(resolve(process.cwd(), "mozconfig"))) {
			writeFileSync(
...
process.on("unhandledException", (err) => errorHandler(err));
process.on("unhandledRejection", (err) => errorHandler(err as any, true));
