	...
	public async exec(cli: Melon) {
		...
		if (!existsSync(resolve(tmpdir(), "melonbuild", "id-lock"))) {
			...
			if (!email || !email.length)
				return lightError(`Invalid Git configuration: user.email`);
			if (!name || !name.length)
				return lightError(`Invalid Git configuration: user.name`);
		...
		let commitDescription = await question(
			`Longer description of this change ${chalk.dim(`(Optional)`)}`
		);
