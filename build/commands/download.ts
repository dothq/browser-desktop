...
export class DownloadCommand {
	...
	public async exec(cli: Melon) {
		...
		if (existsSync(archiveTarPath) || existsSync(archiveDecompressPath)) {
		...
		tar.on("end", async () => {
			...
			if (existsSync(path)) {
				...
				await setConfig(archiveDecompressPath, "core.autocrlf", false);
				$$({
				...
				stream.on("data", async (data) => {
					...
					if (
						filepath &&
						filepath.length &&
						existsSync(resolve(extractPath, data.file))
					) {
						try {
							git.add({
								fs,
				...
				stream.on("end", async () => {
					...
					await move(
						resolve(extractPath, `firefox-${cli.versions.firefox}`),
						outPath
					);
