...
export const fixLineEndings = async () => {
    ...
    await Promise.all(patches.map(async (patch) => {
        ...
        if(existsSync(resolve(process.cwd(), "src", originalPath))) await dispatch("dos2unix", [patch], resolve(process.cwd(), "patches"));
        else log.warning(`Skipping ${patch} as it no longer exists in tree...`);
    }));
}