...
export class BuildCommand {
    ...
    public async exec(cli: Melon) {
        ...
        const runNPMScript = async (name: any) => {
            ...
            if (!script)
                return lightError(
                    `No script found with name \`${name}\`.`
                );
        ...
        const startSccache = async () => {
            return new Promise(async (resolve) => {
                const sccacheRunning = await portInUse(
                    4226
                );
        ...
        if (!!which.sync("sccache", { nothrow: true })) {
            ...
            warning(
                `Could not find \`sccache\`. Builds may be slow!`
            );
        }
