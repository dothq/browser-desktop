...
export class ExportCommand {
    public name = "export";
    public description =
        "Exports all changes from the engine directory.";

    public aliases = ["export-patches", "e"];

    public async exec(cli: Melon) {
        rimraf.sync(patchesDir);

        const { data: gitLog } = await $$({
            cwd: engineDir,
            shutUp: true
        ...
        if (!allCommits.length)
            return warning(
                `No Git history to engine. Perhaps the repository is corrupt?`
            );
        ...
        const patches = readdirSync(patchesDir).filter(
            (f) => f.endsWith(".patch")
        );
