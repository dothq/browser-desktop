...
import { info, infoLevel, lightError, warning } from "./log";
...
const notErrors = ["auto packing the repository", 'see "git help gc"'];
...
const execute = async (strings: any, options?: Options): Promise<Response> => {
	return new Promise((resolve) => {
		const merged = deepMerge(strings);

		// splits by space but preserves quoted items
		const strs = merged.join("").split(/ (?=(?:(?:[^"]*"){2})*[^"]*$)/g);

		const command = strs[0];
		const args = strs.slice(1).map((r: any) => r.replace(/\"/g, ""));
		...
		const handle = (type: any, data: any) => {
			...
			for (let ln of lns) {
				if (strippablePrefixes.includes(ln.split(" ")[0])) {
					for (const prefix of strippablePrefixes) {
						ln = ln.replace(prefix, "").trim();
					}
				}

				if (!!notErrors.find((i) => i.startsWith(ln.toLowerCase()))) {
					type = "info";
				}

				switch (type) {
					case "info":
						if (progress && ln.trim().length) {
							if (!progress?.isSpinning) progress?.start();

							progress.prefixText = infoLevel;
							progress.text = ln;
						} else {
							info(ln);
						}
		...
		proc.stdout?.on("error", (d) => handle("error", d));
		proc.stderr?.on("error", (d) => handle("error", d));

		proc.on("exit", (code: number) => {
			if (progress) progress.end();

			if (code !== 0 && !opts.noErrorKill) {
				lightError("Command failed. See error above.");
