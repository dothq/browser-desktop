...
const main = async () => {
    ...
    if(currentTime > rc.next_merge.beta) {
        ...
        gh.pulls.create({
            ...
            head: "main",
            base: "beta",
            body: `## Changes\n${data.commits.map(commit => { return `* ${commit.commit.message} - @${commit.author.login}` })}`,
            draft: true
        })
    ...
    writeFileSync(resolve(__dirname, "release-channels.json"), JSON.stringify(rc, null, 2))
}
