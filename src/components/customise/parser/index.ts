...
class CustomisableUIParser {
	...
	public isValidWidget(name: string) {
		return gBrowser.customisable.widgets.has(name);
	}

	public isValidLayout(name: string) {
		return gBrowser.customisable.layouts.has(name);
	}
	...
	public compileTree(layout: CustomisableUILayout) {
		...
		for (const value of layout) {
			...
			if (type == "widget") {
				const widget = gBrowser.customisable.widgets.get(
					key
				) as RenderableWidget;
				...
				const subLayout = useStockLayout
					? (gBrowser.customisable.layouts.get(
							key
	...
	public renderFactoryTree(): ReactNode[] {
		 ...
		 * ]
		 */
		const layout = gBrowser.customisable.getLayout(this.id);
		...
		for (const child of children) {
			if (
				...
				class ReactWrapper extends PureComponent {
					public render() {
				...
				reactChildren.push(React.createElement(ReactWrapper));
			} else {
				console.warn(
