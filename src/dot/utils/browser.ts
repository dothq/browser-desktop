...
import { dot } from "../api";
import { AppConstants, BrowserWindowTracker, Cc, Ci, PrivateBrowsingUtils, ReferrerInfo, Services } from "../modules";
...
export const BROWSER_NEW_TAB_URL = window.BROWSER_NEW_TAB_URL;
...
export const whereToOpenLink = (
    ...
    if (alt && dot.prefs.get("browser.altClickSave", false)) {
        return "save";
    }
...
export const getTopWin = (skipPopups?: boolean) => {
    if (
        top.document.documentElement.getAttribute("windowtype") == "navigator:browser" &&
        (
            !skipPopups || 
            top.toolbar.visible
        )
...
export const openLinkIn = (
    ...
    const useOAForPrincipal = (principal: any) => {
        if (principal && principal.isContentPrincipal) {
            const privateBrowsingId = isPrivate || (
                win && 
    ...
    if (
        ...
        if (
            ...
            const sourceTabBrowser = sourceWindow.dot.tabs.selectedTab.webContents;

            const delayedStartupObserver = (subject: any) => {
                if (subject == win) {
                    const browser = win.dot.tabs.selectedTab.webContents;
    ...
    if (where == "current") {
        ...
        if (loadInBackground == null) {
            loadInBackground = fromChrome
                ? false
                : dot.prefs.get("browser.tabs.loadInBackground");
        }
    }
                ...
                Services.obs.notifyObservers(
                    {
                        wrappedJSObject: {
                            ...
                            sourceTabBrowser: win.dot.tabs.selectedTab.webContents,
                            sourceFrameID: frameID,
                        },
                    },
                    "webNavigation-createdNavigationTarget"
