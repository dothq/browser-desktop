import { dot } from "../api";
import { AppConstants, BrowserWindowTracker, ChromeUtils, Ci, PrivateBrowsingUtils, ReferrerInfo, Services } from "../modules";
...
export class BrowserAccess {
    ...
    public _openURIInNewTab(
        ...
        if (window.toolbar.visible) {
            ...
            win = BrowserWindowTracker.getTopWindow({ private: isPrivate });
            needToFocusWin = true;
        }
    ...
    public getContentWindowOrOpenURI(
        ...
        if (
            ...
            throw new Error("openURI did not expect openWindowInfo to be passed if the context is OPEN_EXTERNAL.");
        }
    
        // You should use the --chrome argument instead
        if (isExternal && uri && uri.schemeIs("chrome")) return;
        ...
        if (where == OPEN_DEFAULTWINDOW) {
            if (
                ...
                where = dot.prefs.get("browser.link.open_newwindow");
            }
        }
        ...
        if (flags & OPEN_NO_REFERRER) {
            ...
            referrerInfo = new ReferrerInfo(
                ...
                true,
                makeURI(openWindowInfo.parent.window.location.href)
            );
        } else {
            referrerInfo = new ReferrerInfo(
                Ci.nsIReferrerInfo.EMPTY, 
        ...
        switch (where) {
            ...
            case OPEN_NEWTAB:
                ...
                if (browser) browsingContext = browser.browsingContext;
            ...
            default:
                ...
                if (
                    !dot.prefs.get("browser.tabs.loadDivertedInBackground")
                ) {
                    window.focus();
                }
        }
    ...
    public canClose() {
        ...
        for (const tab of dot.tabs.list) {
            ...
            const { permitUnload } = tab.webContents.permitUnload();
