...
export class UISwitch extends React.Component<UISwitchProps> {
    ...
    public constructor(props: UISwitchProps) {
        ...
        if (this.props.type == "loose") {
            ...
            let type = window.dot.prefs.getType(
                this.props.pref
            );

            if (!type) {
                console.warn(
                    `Preference with ID ${this.props.pref} does not exist.`
                );
            }

            if (type == "bool") {
                this.setState({
                    checked: window.dot.prefs.get(
                        this.props.pref
                    )
                });
            }
    ...
    public onChange(e: any): any {
        if (
            ...
            if (type !== "bool") {
                return console.warn(
                    `Preference with ID ${this.props.pref} is not of type bool.`
                );
            }
