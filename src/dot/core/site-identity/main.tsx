...
export const SiteIdentityMainScreen = ({ tab }: { tab: Tab }) => {
    ...
    let certificate = data.icon == "https" && manager.getCertData();
    ...
    if (data.icon.startsWith("http")) {
        SiteDataManager.updateSites().then(async () => {
            const data = await SiteDataManager.getSites(
                SiteDataManager.getBaseDomainFromHost(tab.urlParsed.host)
            );
    ...
    return (
        <PopupScreen title={`Site information for ${manager.identityHost}`}>
            ...
            {(data.icon == "https" && certificate) && <PopupItem
                icon={"certificate"}
                title={"Certificate"}
                subtitle={`Issued by: ${certificate.issuer || ""}`}
            />}

            {data.icon.startsWith("http") && <PopupItem
                icon={"cookie"}
                title={"Cookies"}
