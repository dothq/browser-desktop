...
export class WikipediaSearchProvider {
    ...
    public description = "Search Wikipedia to find articles on your search."
    public icon = "chrome://dot/content/skin/icons/search/wikipedia.svg"

    public enabled() {
        return dot.prefs.get("dot.search.provider.wikipedia.enabled", true);
    }
    ...
    public constructSuggestUrl(query: string) {
        const lang = dot.utilities.browserLanguage.split("-")[0] || "en";
    ...
    public async suggest(filter: SearchFilter): Promise<SearchSuggestion[]> {
        ...
        const pinkyPromisedFetch = (uri: string): Promise<any> => new Promise((resolve) => {
            NetUtil.asyncFetch({
                uri,
        ...
        const article = await pinkyPromisedFetch(suggestUrl);
        ...
        const viewsPath = `/all-access/all-agents/${article[1][0]}/monthly/${y}${m.toString().padStart(2, "0")}01/${y}${(m + 1).toString().padStart(2, "0")}01`;
