...
export class EngineSearchProvider {
    ...
    public enabled() {
        return dot.prefs.get(
            "dot.search.provider.engine.enabled",
            true
        );
    }
    ...
    public constructSuggestUrl(query: string) {
        const { chrome_settings_overrides } =
            this.currentSearchEngine;
        const provider =
            chrome_settings_overrides.search_provider;

        const getParams =
            provider.suggest_url_get_params.startsWith(
                "?"
            )
        ...
        return substituteSearchUrl({
            url,
            query,
            engine: this.currentSearchEngine
        });
    }
    ...
    public async suggest(
        ...
        const pinkyPromisedFetch = (): Promise<
            string[]
        > =>
            new Promise((resolve) => {
                NetUtil.asyncFetch(
