...
import { AddonManager, FileUtils, OS, Services } from "mozilla";
...
export class BrowserThemes {
    ...
    */
    ...
    public updateAccentColour(value?: string) {
        ...
        if(Object.keys(this.accentColours).includes(value)) {    
            ...
            throw new Error(`Invalid accent colour '${value}'`);
        }
    }
 
    public updateChromeRoundness(value?: any, shouldAnimate?: boolean) {
        if(!value) value = this.browser.preferences.get(
           "dot.ui.roundness"
        , 8);
    ...
    */
    public async load(id?: string) {
        ...
        if (theme) {
            ...
            this.browser.preferences.set("dot.ui.theme", theme.id);
            this.currentThemeId = theme.id;
    
            this.updateAccentColour();
            this.updateChromeRoundness();

            for (const [key, value] of Object.entries(variableMapped)) {
                this.setThemeProperty(
                    key,
                    value
                )
    ...
    public determineDarkness() {
        ...
        const passedAmount = results.filter(i => i == true);
