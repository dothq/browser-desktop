...
export class DotPopupHandlerParent extends JSWindowActorParent {
	...
	async openPopup({
		...
		const win =
			!IS_CONTENT && browsingContext.window ? browsingContext.window : browser.ownerGlobal;
		...
		if (!isNativePopup) {
			popup = /** @type {BrowserContentPopup} */ (
				win.document.createElement("browser-content-popup")
			);
			popup.initialise(
			...
			openName = popup.querySelector(".browser-content-popup-frame").getAttribute("name");
		}

		let urlToLoad = "about:blank";

		if (uri) {
			urlToLoad = Services.io.newURI(uri.spec, "", this.browsingContext.currentURI).spec;
		}
			...
			if (popup) {
				...
				popup.browser.contentWindow.addEventListener("DOMContentLoaded", () => {
					...
					console.log(
						"popup init",
						/** @type {ChromeBrowser} */ (
							popup.browser.contentWindow.gDot.tabs.selectedTab.webContents
						).browsingContext.id
					);
				...
				popupWin.addEventListener("DOMContentLoaded", () => {
					...
					console.log(
						"popup init",
						/** @type {ChromeBrowser} */ (popupWin.gDot.tabs.selectedTab.webContents)
							.browsingContext.id
					);
